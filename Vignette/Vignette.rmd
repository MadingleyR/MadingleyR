...
```{r, eval=FALSE}
# Uninstall MadingleyR package, uncomment code below
...
```
By default the ```madingley_run()``` print simulation process (simulation month), however, it can be useful is some cases to silence the printing of ```madingley_run()``` using ```silenced = TRUE```. Additionally, the ```parallel``` input argument allows the user to run the simulation in serial (on one processing core) or in parallel (using multiple cores). By default the simulation is executed in parallel to speed up the time required to run a simulation. See ```?madingley_run``` for all input arguments.

\pagebreak

## Creating plots

The ```madingley_plot()``` function creates several plots (see figures below) from the outputs generated by ```madingley_run()```. The code below is not run in the vignette, plots are made separately as explained next.
