...
mdata2 = madingley_run(madingley_data = mdata, 
                       ...
                       silenced = TRUE)
```
By default the ```madingley_run()``` print simulation process (simulation month), however, it can be useful is some cases to silence the printing of ```madingley_run()``` using ```silenced = TRUE)```. For example, when making a vignette (saving space) or when running many simulations remotely.
