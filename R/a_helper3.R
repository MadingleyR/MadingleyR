a_helper3 <- function(x0, y0, x1, y1, code=2,
  ...
  if (arr.type=="simple") {
    a_helper3(x0,y0,x1,y1,code=code,length=arr.length/2.54,
           lty=lty, col=col, lwd=lwd, ...)
    return()
  }
  if (arr.type=="T") {
    a_helper3(x0,y0,x1,y1,code=code,length=arr.length/(2*2.54),
  ...
  ## code =3 draws two ah_helper2s
  if (code == 3)
    ah_helper2(x0=xx,y0=yy,angle=angle,
  ...
  ah_helper2(x0=xx,y0=yy,angle=angle,lcol=lcol,arr.col=arr.col,
