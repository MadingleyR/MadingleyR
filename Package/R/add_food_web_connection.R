add_food_web_connection = function(from, to, lwd=2, lty=1, lcol="black", arr.col=lcol,
  ...
  pe_helper4(rx=dst/2,  ry=ry, mid=mid, angle=angle, from = From, to = To,
              lwd=lwd, lty=lty, lcol=lcol)
  ell <- gl_helper1(rx=dst/2, ry=ry, mid=mid, angle=angle,
                    from=1.001*meanpi, to=0.999*meanpi, dr= 0.002)       #Changed from -0.002
  a_helper3(ell[1,1], ell[1,2], ell[nrow(ell),1], ell[nrow(ell),2],
